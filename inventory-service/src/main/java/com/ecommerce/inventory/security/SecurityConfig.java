 ...
 * support. The "roles" claim (a JSON array set by user-service's JwtService) is mapped to
 * ROLE_* authorities so the hasRole()/hasAnyRole() checks below work unchanged.
 */
@Configuration
@EnableWebSecurity
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, JwtDecoder jwtDecoder,
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/v1/inventory/**").hasAnyRole("CUSTOMER", "MANAGER", "ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/v1/inventory/*/stock").hasAnyRole("MANAGER", "ADMIN")
                        // reserve/release are invoked by order-service on behalf of a customer placing/cancelling
                        // an order, so they carry the customer's own token/role, not an elevated one.
                        .requestMatchers(HttpMethod.POST, "/api/v1/inventory/*/reserve", "/api/v1/inventory/*/release")
