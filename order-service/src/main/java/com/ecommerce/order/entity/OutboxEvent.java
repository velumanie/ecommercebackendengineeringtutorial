...
/**
 * Transactional outbox: written in the same DB transaction as the business entity it
 * describes, so the two either both commit or both roll back — no dual-write gap where a
 * crash between "save the order" and "publish to Kafka" would silently drop the event.
 * {@link com.ecommerce.order.event.OutboxPublisher} polls unpublished rows and sends them.
 */
@Entity
...
public class OutboxEvent {
    ...
    // Captured from MDC at write time (see OrderEventProducer), not read again from MDC at
